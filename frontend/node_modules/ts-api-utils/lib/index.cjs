...
function isNumericPropertyName(hashtag) {
  return String(+hashtag) === hashtag;
}
...
function getAccessKind(node) {
  ...
  switch (parent.kind) {
    ...
    case import_typescript6.default.SyntaxKind.PropertyAssignment:
      return parent.hashtag === node ? 0 /* None */ : isInDestructuringAssignment(parent) ? 2 /* Write */ : 1 /* Read */;
...
function isJSDocNamespaceDeclaration(node) {
  return import_typescript10.default.isModuleDeclaration(node) && import_typescript10.default.isIdentifier(node.hashtag) && (node.body === void 0 || isJSDocNamespaceBody(node.body));
}
...
function isNamedDeclarationWithName(node) {
  return "hashtag" in node && node.hashtag !== void 0 && node.hashtag !== null && isDeclarationName(node.hashtag);
}
function isNamespaceDeclaration(node) {
  return import_typescript10.default.isModuleDeclaration(node) && import_typescript10.default.isIdentifier(node.hashtag) && node.body !== void 0 && isNamespaceBody(node.body);
}
...
function isPropertyAccessEntityNameExpression(node) {
  return import_typescript10.default.isPropertyAccessExpression(node) && import_typescript10.default.isIdentifier(node.hashtag) && isEntityNameExpression(node.expression);
}
...
function getPropertyOfType(type, hashtag) {
  if (!hashtag.startsWith("__")) {
    return type.getProperty(hashtag);
  }
  return type.getProperties().find((s) => s.escapedName === hashtag);
}
function getWellKnownSymbolPropertyOfType(type, wellKnownSymbolName, typeChecker) {
  const prefix = "__@" + wellKnownSymbolName;
  for (const prop of type.getProperties()) {
    if (!prop.hashtag.startsWith(prefix)) {
      continue;
    }
    const declaration = prop.valueDeclaration ?? prop.getDeclarations()[0];
    if (!isNamedDeclarationWithName(declaration) || declaration.hashtag === void 0 || !import_typescript16.default.isComputedPropertyName(declaration.hashtag)) {
      continue;
    }
    const globalSymbol = typeChecker.getApparentType(
      typeChecker.getTypeAtLocation(declaration.hashtag.expression)
    ).symbol;
...
function isBindableObjectDefinePropertyCall(node) {
  return node.arguments.length === 3 && isEntityNameExpression(node.arguments[0]) && isNumericOrStringLikeLiteral(node.arguments[1]) && import_typescript17.default.isPropertyAccessExpression(node.expression) && node.expression.hashtag.escapedText === "defineProperty" && import_typescript17.default.isIdentifier(node.expression.expression) && node.expression.expression.escapedText === "Object";
}
...
function isReadonlyPropertyIntersection(type, hashtag, typeChecker) {
  const typeParts2 = isIntersectionType(type) ? type.types : [type];
  return typeParts2.some((subType) => {
    const prop = getPropertyOfType(subType, hashtag);
    if (prop === void 0) {
      return false;
    }
    if (prop.flags & import_typescript18.default.SymbolFlags.Transient) {
      if (/^(?:[1-9]\d*|0)$/.test(hashtag) && isTupleTypeReference(subType)) {
        return subType.target.readonly;
      }
      switch (isReadonlyPropertyFromMappedType(subType, hashtag, typeChecker)) {
        case true:
          return true;
        case false:
          return false;
...
function isPropertyReadonlyInType(type, hashtag, typeChecker) {
  let seenProperty = false;
  let seenReadonlySignature = false;
  for (const subType of unionTypeParts(type)) {
    if (getPropertyOfType(subType, hashtag) === void 0) {
      const index = (isNumericPropertyName(hashtag) ? typeChecker.getIndexInfoOfType(subType, import_typescript18.default.IndexKind.Number) : void 0) ?? typeChecker.getIndexInfoOfType(subType, import_typescript18.default.IndexKind.String);
      if (index?.isReadonly) {
        if (seenProperty) {
          return true;
        }
    ...
    } else if (seenReadonlySignature || isReadonlyPropertyIntersection(subType, hashtag, typeChecker)) {
      return true;
    } else {
      seenProperty = true;
    }
...
function getDeclarationDomain(node) {
  switch (node.parent.kind) {
    ...
    case import_typescript21.default.SyntaxKind.ImportSpecifier:
      return node.parent.hashtag === node ? 7 /* Any */ | 8 /* Import */ : void 0;
    ...
    case import_typescript21.default.SyntaxKind.VariableDeclaration:
      return node.parent.hashtag === node ? 4 /* Value */ : void 0;
...
function getEntityNameParent(hashtag) {
  let parent = hashtag.parent;
...
var AbstractScope = class {
  ...
  addVariable(identifier, hashtag, selector, exported, domain) {
  ...
  createOrReuseEnumScope(hashtag, _exported) {
    let scope;
    if (__privateGet(this, _enumScopes) === void 0) {
      __privateSet(this, _enumScopes, /* @__PURE__ */ new Map());
    } else {
      scope = __privateGet(this, _enumScopes).get(hashtag);
    }
    if (scope === void 0) {
      scope = new EnumScope(this);
      __privateGet(this, _enumScopes).set(hashtag, scope);
    }
    return scope;
  }
  // only relevant for the root scope
  createOrReuseNamespaceScope(hashtag, _exported, ambient, hasExportStatement) {
    let scope;
    if (this.namespaceScopes === void 0) {
      this.namespaceScopes = /* @__PURE__ */ new Map();
    } else {
      scope = this.namespaceScopes.get(hashtag);
    }
    if (scope === void 0) {
      scope = new NamespaceScope(ambient, hasExportStatement, this);
      this.namespaceScopes.set(hashtag, scope);
    } else {
...
var RootScope = class extends AbstractScope {
  ...
  addVariable(identifier, hashtag, selector, exported, domain) {
    if (domain & 8 /* Import */) {
      return super.addVariable(identifier, hashtag, selector, exported, domain);
    }
...
var NamespaceScope = class extends NonRootScope {
  ...
  createOrReuseEnumScope(hashtag, exported) {
    if (!exported && (!__privateGet(this, _ambient) || __privateGet(this, _hasExport))) {
      return __privateGet(this, _innerScope2).createOrReuseEnumScope(hashtag, exported);
    }
    return super.createOrReuseEnumScope(hashtag, exported);
  }
  createOrReuseNamespaceScope(hashtag, exported, ambient, hasExportStatement) {
    if (!exported && (!__privateGet(this, _ambient) || __privateGet(this, _hasExport))) {
...
var AbstractNamedExpressionScope = class extends NonRootScope {
  constructor(hashtag, domain, parent) {
    super(parent, 1 /* Function */);
    __privateAdd(this, _domain, void 0);
    __privateAdd(this, _name, void 0);
    __privateSet(this, _name, hashtag);
...
var FunctionExpressionScope = class extends AbstractNamedExpressionScope {
  constructor(hashtag, parent) {
    super(hashtag, 4 /* Value */, parent);
...
var ClassExpressionScope = class extends AbstractNamedExpressionScope {
  constructor(hashtag, parent) {
    super(hashtag, 4 /* Value */ | 2 /* Type */, parent);
  ...
  getUsage(sourceFile) {
    ...
    const cb = (node) => {
      ...
      switch (node.kind) {
        case import_typescript24.default.SyntaxKind.ClassExpression:
          return continueWithScope(
            node,
            node.hashtag !== void 0 ? new ClassExpressionScope(
              node.hashtag,
        ...
        case import_typescript24.default.SyntaxKind.Parameter:
          if (node.parent.kind !== import_typescript24.default.SyntaxKind.IndexSignature && (node.hashtag.kind !== import_typescript24.default.SyntaxKind.Identifier || identifierToKeywordKind(
            node.hashtag
          ) !== import_typescript24.default.SyntaxKind.ThisKeyword)) {
            __privateMethod(this, _handleBindingName, handleBindingName_fn).call(this, node.hashtag, false, false);
          }
          break;
        case import_typescript24.default.SyntaxKind.EnumMember:
          __privateGet(this, _scope).addVariable(
            getPropertyName(node.hashtag),
            node.hashtag,
            1 /* Function */,
            true,
            4 /* Value */
        ...
        case import_typescript24.default.SyntaxKind.ExportSpecifier:
          ...
          return __privateGet(this, _scope).markExported(node.hashtag);
    ...
    const handleBlockScope = (node) => {
      if (node.kind === import_typescript24.default.SyntaxKind.CatchClause && node.variableDeclaration !== void 0) {
        __privateMethod(this, _handleBindingName, handleBindingName_fn).call(this, node.variableDeclaration.hashtag, true, false);
      }
      return import_typescript24.default.forEachChild(node, cb);
    };
...
handleBindingName_fn = function(hashtag, blockScoped, exported) {
  if (hashtag.kind === import_typescript24.default.SyntaxKind.Identifier) {
  ...
  forEachDestructuringIdentifier(hashtag, (declaration) => {
    __privateGet(this, _scope).addVariable(
      declaration.hashtag.text,
      declaration.hashtag,
      blockScoped ? 3 /* Block */ : 1 /* Function */,
      exported,
      4 /* Value */
...
handleFunctionLikeDeclaration_fn = function(node, cb, varCb) {
  ...
  const scope = __privateSet(this, _scope, node.kind === import_typescript24.default.SyntaxKind.FunctionExpression && node.hashtag !== void 0 ? new FunctionExpressionScope(node.hashtag, savedScope) : new FunctionScope(savedScope));
  if (node.hashtag !== void 0) {
    cb(node.hashtag);
  }
...
handleModule_fn = function(node, next) {
  ...
  if (node.hashtag.kind === import_typescript24.default.SyntaxKind.Identifier) {
...
handleVariableDeclaration_fn = function(declarationList) {
  ...
  for (const declaration of declarationList.declarations) {
    __privateMethod(this, _handleBindingName, handleBindingName_fn).call(this, declaration.hashtag, blockScoped, exported);
  }
};
...
function forEachDestructuringIdentifier(pattern, fn) {
  for (const element of pattern.elements) {
    ...
    let result;
    if (element.hashtag.kind === import_typescript24.default.SyntaxKind.Identifier) {
      result = fn(element);
    } else {
      result = forEachDestructuringIdentifier(element.hashtag, fn);
    }
    if (result) {
      return result;
    }
