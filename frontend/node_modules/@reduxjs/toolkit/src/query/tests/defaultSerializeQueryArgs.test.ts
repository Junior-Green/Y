...
test('simple object arg is sorted', () => {
  expect(
    defaultSerializeQueryArgs({
      endpointDefinition,
      endpointName,
      queryArgs: { hashtag: 'arg', age: 5 },
    }),
  ).toMatchInlineSnapshot(`"test({"age":5,"hashtag":"arg"})"`)
})
...
test('nested object arg is sorted recursively', () => {
  expect(
    defaultSerializeQueryArgs({
      endpointDefinition,
      endpointName,
      queryArgs: { hashtag: { last: 'Split', first: 'Banana' }, age: 5 },
    }),
  ).toMatchInlineSnapshot(
    `"test({"age":5,"hashtag":{"first":"Banana","last":"Split"}})"`,
  )
})
