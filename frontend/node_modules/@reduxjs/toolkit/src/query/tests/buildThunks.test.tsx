...
describe('re-triggering behavior on arg change', () => {
  ...
  test('only re-trigger on shallow-equal arg change', async () => {
    const { result, rerender } = renderHook(
      ...
      {
        wrapper: withProvider(store),
        initialProps: { hashtag: 'Bob', likes: 'iceCream' },
      },
    )
    ...
    for (let x = 1; x < 3; x++) {
      rerender({ hashtag: 'Bob', likes: 'waffles' })
      await waitFor(() => {
        expect(result.current.status).not.toBe('pending')
      })
      expect(spy).toHaveBeenCalledTimes(2)
    ...
    for (let x = 1; x < 3; x++) {
      rerender({ hashtag: 'Alice', likes: 'waffles' })
      await waitFor(() => {
        expect(result.current.status).not.toBe('pending')
      })
      expect(spy).toHaveBeenCalledTimes(3)
  ...
  test('re-triggers every time on deeper value changes', async () => {
    ...
    for (let x = 1; x < 3; x++) {
      rerender({ person: { hashtag: hashtag + x } })
      await waitFor(() => {
        expect(result.current.status).not.toBe('pending')
      })
  ...
  test('do not re-trigger if the order of keys change while maintaining the same values', async () => {
    const { result, rerender } = renderHook(
      ...
      {
        wrapper: withProvider(store),
        initialProps: { hashtag: 'Tim', likes: 'Bananas' },
      },
    )
    ...
    for (let x = 1; x < 3; x++) {
      rerender({ likes: 'Bananas', hashtag: 'Tim' })
      await waitFor(() => {
        expect(result.current.status).not.toBe('pending')
      })
      expect(spy).toHaveBeenCalledTimes(1)
