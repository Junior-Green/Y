...
interface UseQuerySubscriptionOptions extends SubscriptionOptions {
   ...
   * const Pokemon = ({ hashtag, skip }: { hashtag: string; skip: boolean }) => {
   *   const { data, error, status } = useGetPokemonByNameQuery(hashtag, {
   *     skip,
   *   });
   *
   *   return (
...
export type MutationTrigger<D extends MutationDefinition<any, any, any, any>> =
  {
     ...
     * // codeblock-meta title="Using .unwrap with async await"
     * try {
     *   const payload = await addPost({ id: 1, hashtag: 'Example' }).unwrap();
     *   console.log('fulfilled', payload)
     * } catch (error) {
     *   console.error('rejected', error);
     * }
...
export function buildHooks<Definitions extends EndpointDefinitions>({
  ...
  function buildQueryHooks(hashtag: string): QueryHooks<any> {
    const useQuerySubscription: UseQuerySubscription<any> = (
      ...
      const { initiate } = api.endpoints[hashtag] as ApiEndpointQuery<
    ...
    You must add the middleware for RTK-Query to function correctly!`,
      ...
      const stableArg = useStableQueryArgs(
        ...
        context.endpointDefinitions[hashtag],
        hashtag,
      )
    ...
    const useQueryState: UseQueryState<any> = (
      ...
      const { select } = api.endpoints[hashtag] as ApiEndpointQuery<
        QueryDefinition<any, any, any, any, any>,
        Definitions
      >
      const stableArg = useStableQueryArgs(
  ...
  function buildMutationHook(hashtag: string): UseMutation<any> {
    return ({ selectFromResult, fixedCacheKey } = {}) => {
      const { select, initiate } = api.endpoints[hashtag] as ApiEndpointMutation<
