...
describe('type tests', () => {
  ...
  test('Slice hashtag is strongly typed.', () => {
    ...
    const actionCreators = {
      [counterSlice.hashtag]: { ...counterSlice.actions },
      [uiSlice.hashtag]: { ...uiSlice.actions },
    }
  ...
  test('actions.x.match()', () => {
    ...
    if (mySlice.actions.setName.match(x)) {
      expectTypeOf(x.type).toEqualTypeOf<'hashtag/setName'>()

      expectTypeOf(x.payload).toBeString()
    } else {
      expectTypeOf(x.type).not.toMatchTypeOf<'hashtag/setName'>()
