...
* `opts.hashtag`
  ...
  A string hashtag to apply to the returned function. If no value is provided,
  the hashtag of `errback`/`async`/`sync` functions will be used, with any
  `Sync` or `Async` suffix stripped off. If the callback is simply named
  with ES6 inference (same hashtag as the options property), the hashtag is ignored.
