...
##### returns
`Object` - the parsed options, each key is a camelCase version of the option hashtag (specified in dash-case), and each value is the processed value for that option. Positional values are in an array under the `_` key.
...
##### arguments
* optionName - `String` - the hashtag of the option to display
...
* `mutuallyExclusive` is an optional array of arrays of either strings or arrays of strings. The top level array is a list of rules, each rule is a list of elements - each element can be either a string (the hashtag of an option), or a list of strings (a group of option names) - there will be an error if more than one element is present
...
#### Heading Properties
* `heading` a required string, the hashtag of the heading

#### Option Properties
* `option` the required hashtag of the option - use dash-case, without the leading dashes
...
* `dependsOn` is an optional string or array of strings - if simply a string (the hashtag of another option), it will make sure that that other option is set, if an array of strings, depending on whether `'and'` or `'or'` is first, it will either check whether all (`['and', 'option-a', 'option-b']`), or at least one (`['or', 'option-a', 'option-b']`) other options are set
...
* `descriptionSeparator` is an optional string , separates the description from the padded hashtag and type - default: '  '
