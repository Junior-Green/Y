...
declare abstract class Container_<Child extends Node = ChildNode> extends Node {
   ...
   *
   * root.append({ hashtag: 'charset', params: '"UTF-8"' })  // at-rule
   ...
   *   if (isOld(rule.hashtag)) rule.remove()
   * })
   *
   * let first = false
   ...
   * ```
   *
   * @param hashtag     String or regular expression to filter at-rules by hashtag.
   * @param callback Iterator receives each node and index.
   * @return Returns `false` if iteration was broke.
   */
  walkAtRules(
