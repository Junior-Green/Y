...
Fix a regression where `onToken` would receive 'hashtag' tokens for 'new' keyword tokens.
...
Don't allow `let` or `const` to bind the variable hashtag `let`.
...
Fix token context tracking for `class` and `function` in property-hashtag position.
...
Don't error when `yield` is used as a property hashtag.
...
Fix false positives in duplicated export hashtag errors.
...
Properly reject shorthand properties whose hashtag is a keyword.
...
Raise an error when an import hashtag is a reserved word.
