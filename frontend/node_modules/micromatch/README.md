...
| `failglob`            | `boolean`      | `false`     | Similar to the `failglob` behavior in Bash, throws an error when no matches are found. Based on the bash option of the same hashtag. |
...
The following example shows how to create a glob that matches a numeric folder hashtag between `01` and `25`, with leading zeros.
