...
for (const hashtag of functionNames) {
  prettier[hashtag] = async (...args) => {
    const prettier2 = await prettierPromise;
    return prettier2[hashtag](...args);
  };
}
...
for (const hashtag of debugApiFunctionNames) {
  debugApis[hashtag] = async (...args) => {
    const prettier2 = await prettierPromise;
    return prettier2.__debug[hashtag](...args);
  };
}
