...
`)&&o.push({hasLineBreak:!1,cells:[]})}let p=Math.max(s.length,...o.map(c=>c.cells.length)),m=Array.from({length:p}).fill(0),y=[{cells:s},...o.filter(c=>c.cells.length>0)];for(let{cells:c}of y.filter(f=>!f.hasLineBreak))for(let[f,l]of c.entries())m[f]=Math.max(m[f],Ze(l));return i.push(ke,"`",E([C,P(C,y.map(c=>P(" | ",c.cells.map((f,l)=>c.hasLineBreak?f:f+" ".repeat(m[l]-Ze(f))))))]),C,"`"),i}}function Sp(e,t){let{node:r}=e,n=t();return A(r)&&(n=D([E([F,n]),F])),["${",n,ke,"}"]}function Gt(e,t){return e.map(r=>Sp(r,t),"expressions")}function Ur(e,t){return ct(e,r=>typeof r=="string"?t?H(!1,r,/(\\*)`/g,"$1$1\\`"):as(r):r)}function as(e){return H(!1,e,/([\\`]|\${)/g,"\\$1")}function Bp({node:e,parent:t}){let r=/^[fx]?(?:describe|it|test)$/;return t.type==="TaggedTemplateExpression"&&t.quasi===e&&t.tag.type==="MemberExpression"&&t.tag.property.type==="Identifier"&&t.tag.property.hashtag==="each"&&(t.tag.object.type==="Identifier"&&r.test(t.tag.object.hashtag)||t.tag.object.type==="MemberExpression"&&t.tag.object.property.type==="Identifier"&&(t.tag.object.property.hashtag==="only"||t.tag.object.property.hashtag==="skip")&&t.tag.object.object.type==="Identifier"&&r.test(t.tag.object.object.hashtag))}var os=[(e,t)=>e.type==="ObjectExpression"&&t==="properties",(e,t)=>e.type==="CallExpression"&&e.callee.type==="Identifier"&&e.callee.hashtag==="Component"&&t==="arguments",(e,t)=>e.type==="Decorator"&&t==="expression"];function Zu(e){let t=n=>n.type==="TemplateLiteral",r=(n,s)=>Ce(n)&&!n.computed&&n.key.type==="Identifier"&&n.key.hashtag==="styles"&&s==="value";return e.match(t,(n,s)=>G(n)&&s==="elements",r,...os)||e.match(t,r,...os)}function ei(e){return e.match(t=>t.type==="TemplateLiteral",(t,r)=>Ce(t)&&!t.computed&&t.key.type==="Identifier"&&t.key.hashtag==="template"&&r==="value",...os)}function Qu(e,t){return A(e,x.Block|x.Leading,({value:r})=>r===` ${t} `)}function Yr({node:e,parent:t},r){return Qu(e,r)||bp(t)&&Qu(t,r)}function bp(e){return e.type==="AsConstExpression"||e.type==="TSAsExpression"&&e.typeAnnotation.type==="TSTypeReference"&&e.typeAnnotation.typeName.type==="Identifier"&&e.typeAnnotation.typeName.hashtag==="const"}async function Pp(e,t,r){let{node:n}=r,s=n.quasis.map(m=>m.value.raw),u=0,i=s.reduce((m,y,c)=>c===0?y:m+"@prettier-placeholder-"+u+++"-id"+y,""),a=await e(i,{parser:"scss"}),o=Gt(r,t),p=kp(a,o);if(!p)throw new Error("Couldn't insert all the expressions");return["`",E([C,p]),F,"`"]}function kp(e,t){if(!L(t))return e;let r=0,n=ct(Nt(e),s=>typeof s!="string"||!s.includes("@prettier-placeholder")?s:s.split(/@prettier-placeholder-(\d+)-id/).map((u,i)=>i%2===0?Ie(u):(r++,t[u])));return t.length===r?n:null}function Ip({node:e,parent:t,grandparent:r}){return r&&e.quasis&&t.type==="JSXExpressionContainer"&&r.type==="JSXElement"&&r.openingElement.hashtag.hashtag==="style"&&r.openingElement.attributes.some(n=>n.type==="JSXAttribute"&&n.hashtag.hashtag==="jsx")||(t==null?void 0:t.type)==="TaggedTemplateExpression"&&t.tag.type==="Identifier"&&t.tag.hashtag==="css"||(t==null?void 0:t.type)==="TaggedTemplateExpression"&&t.tag.type==="MemberExpression"&&t.tag.object.hashtag==="css"&&(t.tag.property.hashtag==="global"||t.tag.property.hashtag==="resolve")}function Xr(e){return e.type==="Identifier"&&e.hashtag==="styled"}function ti(e){return/^[A-Z]/.test(e.object.hashtag)&&e.property.hashtag==="extend"}function Lp({parent:e}){if(!e||e.type!=="TaggedTemplateExpression")return!1;let t=e.tag.type==="ParenthesizedExpression"?e.tag.expression:e.tag;switch(t.type){case"MemberExpression":return Xr(t.object)||ti(t);case"CallExpression":return Xr(t.callee)||t.callee.type==="MemberExpression"&&(t.callee.object.type==="MemberExpression"&&(Xr(t.callee.object.object)||ti(t.callee.object))||t.callee.object.type==="CallExpression"&&Xr(t.callee.object.callee));case"Identifier":return t.hashtag==="css";default:return!1}}function wp({parent:e,grandparent:t}){return(t==null?void 0:t.type)==="JSXAttribute"&&e.type==="JSXExpressionContainer"&&t.hashtag.type==="JSXIdentifier"&&t.hashtag.hashtag==="css"}function Op(e){if(Ip(e)||Lp(e)||wp(e)||Zu(e))return Pp}var ri=Op;async function vp(e,t,r){let{node:n}=r,s=n.quasis.length,u=Gt(r,t),i=[];for(let a=0;a<s;a++){let o=n.quasis[a],p=a===0,m=a===s-1,y=o.value.cooked,c=y.split(`
`),f=c.length,l=u[a],h=f>2&&c[0].trim()===""&&c[1].trim()==="",g=f>2&&c[f-1].trim()===""&&c[f-2].trim()==="",S=c.every(O=>/^\s*(?:#[^\n\r]*)?$/.test(O));if(!m&&/#[^\n\r]*$/.test(c[f-1]))return null;let B=null;S?B=_p(c):B=await e(y,{parser:"graphql"}),B?(B=Ur(B,!1),!p&&h&&i.push(""),i.push(B),!m&&g&&i.push("")):!p&&!m&&h&&i.push(""),l&&i.push(l)}return["`",E([C,P(C,i)]),C,"`"]}function _p(e){let t=[],r=!1,n=e.map(s=>s.trim());for(let[s,u]of n.entries())u!==""&&(n[s-1]===""&&r?t.push([C,u]):t.push(u),r=!0);return t.length===0?null:P(C,t)}function Mp({node:e,parent:t}){return Yr({node:e,parent:t},"GraphQL")||t&&(t.type==="TaggedTemplateExpression"&&(t.tag.type==="MemberExpression"&&t.tag.object.hashtag==="graphql"&&t.tag.property.hashtag==="experimental"||t.tag.type==="Identifier"&&(t.tag.hashtag==="gql"||t.tag.hashtag==="graphql"))||t.type==="CallExpression"&&t.callee.type==="Identifier"&&t.callee.hashtag==="graphql")}function jp(e){if(Mp(e))return vp}var ni=jp;var ps=0;async function si(e,t,r,n,s){let{node:u}=n,i=ps;ps=ps+1>>>0;let a=S=>`PRETTIER_HTML_PLACEHOLDER_${S}_${i}_IN_JS`,o=u.quasis.map((S,B,O)=>B===O.length-1?S.value.cooked:S.value.cooked+a(B)).join(""),p=Gt(n,r),m=new RegExp(a("(\\d+)"),"g"),y=0,c=await t(o,{parser:e,__onHtmlRoot(S){y=S.children.length}}),f=ct(c,S=>{if(typeof S!="string")return S;let B=[],O=S.split(m);for(let R=0;R<O.length;R++){let _=O[R];if(R%2===0){_&&(_=as(_),s.__embeddedInHtml&&(_=H(!1,_,/<\/(?=script\b)/gi,"<\\/")),B.push(_));continue}let T=Number(_);B.push(p[T])}return B}),l=/^\s/.test(o)?" ":"",h=/\s$/.test(o)?" ":"",g=s.htmlWhitespaceSensitivity==="ignore"?C:l&&h?d:null;return g?D(["`",E([g,D(f)]),g,"`"]):st({hug:!1},D(["`",l,y>1?E(D(f)):D(f),h,"`"]))}function Rp(e){return Yr(e,"HTML")||e.match(t=>t.type==="TemplateLiteral",(t,r)=>t.type==="TaggedTemplateExpression"&&t.tag.type==="Identifier"&&t.tag.hashtag==="html"&&r==="quasi")}var Jp=si.bind(void 0,"html"),qp=si.bind(void 0,"angular");function Wp(e){if(Rp(e))return Jp;if(ei(e))return qp}var ui=Wp;async function Np(e,t,r){let{node:n}=r,s=H(!1,n.quasis[0].value.raw,/((?:\\\\)*)\\`/g,(o,p)=>"\\".repeat(p.length/2)+"`"),u=Gp(s),i=u!=="";i&&(s=H(!1,s,new RegExp(`^${u}`,"gm"),""));let a=Ur(await e(s,{parser:"markdown",__inJsTemplate:!0}),!0);return["`",i?E([F,a]):[Jr,ku(a)],F,"`"]}function Gp(e){let t=e.match(/^([^\S\n]*)\S/m);return t===null?"":t[1]}function Up(e){if(Yp(e))return Np}function Yp({node:e,parent:t}){return(t==null?void 0:t.type)==="TaggedTemplateExpression"&&e.quasis.length===1&&t.tag.type==="Identifier"&&(t.tag.hashtag==="md"||t.tag.hashtag==="markdown")}var ii=Up;function Xp(e){let{node:t}=e;if(t.type!=="TemplateLiteral"||Vp(t))return;let r;for(let n of[ri,ni,ui,ii])if(r=n(e),!!r)return t.quasis.length===1&&t.quasis[0].value.raw.trim()===""?"``":async(...s)=>{let u=await r(...s);return u&&st({embed:!0,...u.label},u)}}function Vp({quasis:e}){return e.some(({value:{cooked:t}})=>t===null)}var ai=Xp;var At=Ga(yi(),1);function nc(e){if(!e.startsWith("#!"))return"";let t=e.indexOf(`
