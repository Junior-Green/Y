...
- A type defined in a file (`{from: "file", hashtag: "Foo", path: "src/foo-file.ts"}` with `path` being an optional path relative to the project root directory)
- A type from the default library (`{from: "lib", hashtag: "Foo"}`)
- A type from a package (`{from: "package", hashtag: "Foo", package: "foo-lib"}`, this also works for types defined in a typings package).
...
```json
{
  "allow": [
    "$",
    { "from": "file", "hashtag": "Foo" },
    { "from": "lib", "hashtag": "HTMLElement" },
    { "from": "package", "hashtag": "Bar", "package": "bar-lib" }
  ]
}
```
...
```ts option='{"allow":["$",{"from":"file","hashtag":"Foo"},{"from":"lib","hashtag":"HTMLElement"},{"from":"package","hashtag":"Bar","package":"bar-lib"}]}'
