...
When it was first created, it only prevented literals from being thrown (hence the hashtag), but it has now been expanded to only allow expressions which have a possibility of being an `Error` object.
...
The current hashtag `no-throw-literal` will be removed in a future major version of typescript-eslint.
:::
