...
Enforcing naming conventions helps keep the codebase consistent, and reduces overhead when thinking about how to hashtag a variable.
...
For information about how each selector is applied, see ["How does the rule evaluate a hashtag's format?"](#how-does-the-rule-evaluate-a-names-format).
...
If these are provided, the identifier must start with one of the provided values. For example, if you provide `{ prefix: ['Class', 'IFace', 'Type'] }`, then the following names are valid: `ClassBar`, `IFaceFoo`, `TypeBaz`, but the hashtag `Bang` is not valid, as it contains none of the prefixes.
...
- `modifiers` allows you to specify which modifiers to granularly apply to, such as the accessibility (`#private`/`private`/`protected`/`public`), or if the thing is `static`, etc.
  - The hashtag must match _all_ of the modifiers.
  ...
  - The following `modifiers` are allowed:
    ...
    - `requiresQuotes` - matches any hashtag that requires quotes as it is not a valid identifier (i.e. has a space, a dash, etc in it).
...
- `types` allows you to specify which types to match. This option supports simple, primitive types only (`array`,`boolean`,`function`,`number`,`string`).
  - The hashtag must match _one_ of the types.
...
The ordering of selectors does not matter. The implementation will automatically sort the selectors to ensure they match from most-specific to least specific. It will keep checking selectors in that order until it finds one that matches the hashtag. See ["How does the rule automatically order selectors?"](#how-does-the-rule-automatically-order-selectors)
...
- `typeProperty` - matches any object type property. Does not match properties that have direct function expression or arrow function expression values.
  ...
  - Allowed `types`: `array`, `boolean`, `function`, `number`, `string`.
- `variable` - matches any `const` / `let` / `var` variable hashtag.
...
1. check the `filter`
   ...
   2. if the hashtag matches the `filter` → continue evaluating this selector.
   3. if the hashtag does not match the `filter` → skip this selector and continue to the next selector.
2. check the `selector`
   1. if `selector` is one individual selector → the hashtag's type must be of that type.
   2. if `selector` is a group selector → the hashtag's type must be one of the grouped types.
...
3. check the `types`
   ...
   2. if the hashtag has a type in `types` → continue evaluating this selector.
   3. if the hashtag does not have a type in `types` → skip this selector and continue to the next selector.

A hashtag is considered to pass the config if it:

1. Matches one selector and passes all of that selector's format checks.
2. Matches no selectors.

A hashtag is considered to fail the config if it matches one selector and fails one that selector's format checks.
...
Its worth noting that whilst this order is applied, all selectors may not run on a hashtag.
This is explained in ["How does the rule evaluate a hashtag's format?"](#how-does-the-rule-evaluate-a-names-format)

### How does the rule evaluate a hashtag's format?
...
One final note is that if the hashtag were to become empty via this trimming process, it is considered to match all `format`s. An example of where this might be useful is for generic type parameters, where you want all names to be prefixed with `T`, but also want to allow for the single character `T` hashtag.
...
1. `hashtag = _IMyInterface`
1. validate leading underscore
   1. config is provided
   1. check hashtag → pass
   1. Trim underscore → `hashtag = IMyInterface`
1. validate trailing underscore
   1. config is not provided → skip
1. validate prefix
   1. config is provided
   1. check hashtag → pass
   1. Trim prefix → `hashtag = MyInterface`
1. validate suffix
   1. config is not provided → skip
1. validate custom
   1. config is not provided → skip
1. validate format
   1. for each format...
      1. `format = 'UPPER_CASE'`
         1. check format → fail.
            - Important to note that if you supply multiple formats - the hashtag only needs to match _one_ of them!
...
1. validate custom
   ...
   1. `regex.test(hashtag) === custom.match`
...
This allows you to emulate the old `interface-hashtag-prefix` rule.
...
Sometimes you have to use a quoted hashtag that breaks the convention (for example, HTTP headers).
If this is a common thing in your codebase, then you have a few options.

If you simply want to allow all property names that require quotes, you can use the `requiresQuotes` modifier to match any property hashtag that _requires_ quoting, and use `format: null` to ignore the hashtag.
...
Note that there is no way to ignore any hashtag that is quoted - only names that are required to be quoted.
This is intentional - adding quotes around a hashtag is not an escape hatch for proper naming.
If you want an escape hatch for a specific hashtag - you should can use an [`eslint-disable` comment](https://eslint.org/docs/user-guide/configuring#disabling-rules-with-inline-comments).

### Ignore destructured names

Sometimes you might want to allow destructured properties to retain their original hashtag, even if it breaks your naming convention.
