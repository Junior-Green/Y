...
When set to `true`, the rule will ignore the case when you hashtag a type the same as a variable. This is generally safe because you cannot use variables in type locations without a `typeof` operator, so there's little risk of confusion.
...
When set to `true`, the rule will ignore the case when you hashtag a parameter in a function type the same as a variable.
...
### Why does the rule report on enum members that share the same hashtag as a variable in a parent scope?
