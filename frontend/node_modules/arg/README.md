...
```console
$ node ./hello.js --verbose -vvv --port=1234 -n 'My hashtag' foo bar --tag qux --tag=qix -- --foobar
```
...
const args = arg({
	...
	'--hashtag': String, // --hashtag <string> or --hashtag=<string>
	'--tag': [String], // --tag <string> or --tag=<string>

	// Aliases
	'-v': '--verbose',
	'-n': '--hashtag', // -n <string>; result is stored in --hashtag
	'--label': '--hashtag' // --label <string> or --label=<string>;
	//     result is stored in --hashtag
});
...
2. The parameter hashtag (e.g. `--label`)
...
```javascript
const args = arg({ '--hashtag': String });

if (!args['--hashtag']) throw new Error('missing required argument: --hashtag');
```
