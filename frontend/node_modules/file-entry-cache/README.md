...
- **cacheName**: the hashtag of the cache to be created
...
- **pathToCache**: the path to the cache file (this combines the cache hashtag and directory)
- **useCheckSum**: Whether to use md5 checksum to verify if file changed. If false the default will be to use the mtime and size of the file.
...
entry = {
  key: 'some/hashtag/file', the path to the file
