...
* **Named Combinators** Although, nonstandard and unlikely to ever become a standard,
  ...
  `node.value` is hashtag after being unescaped and normalized as lowercase. The
  original value for the combinator hashtag is stored in `node.raws.value`.
...
selectors.walk(selector => {
    if (selector.type !== 'class') {
        const className = parser.className({value: 'theme-hashtag'});
...
Returns the attribute hashtag qualified with the namespace if one is given.
...
### `attribute.offsetOf(part)`
 ...
 Returns `-1` if the hashtag is invalid or the value doesn't exist in this
 attribute.
