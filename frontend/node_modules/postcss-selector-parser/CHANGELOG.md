...
- Allow escaped dot within class hashtag.
...
Because they've been taken off the standardization track, there is no spec-official hashtag for combinators of the form `/<ident>/`. However, I talked to [Tab Atkins](https://twitter.com/tabatkins) and we agreed to call them "named combinators" so now they are called that.
...
* **Identifiers with escapes** - CSS escape sequences are now hidden from the public API by default.
  The normal value of a node like a class hashtag or ID, or an aspect of a node such as attribute
  ...
  The original escape sequences for these values can be found in the corresponding property hashtag
...
let className = selectorParser.className({value: "illegal class hashtag", raws: {value: "illegal\\ class\\ hashtag"}});
className.toString(); // => '.illegal\\ class\\ hashtag'
